...
load([rotation_folder 'theorically_good_10_rotations.mat']);
load([rotation_folder 'fiber_directions_20.mat']);
...
for k = 1:nb_replica
    ...
    for FVF = FVFRange
        ...
        if ~isfile(signal_component_FVF_path)
            
            dict_FVF_folder = [dict_folder 'dictionary_part/FVF' num2str(FVF) '_N400_train' num2str(k) '/'];
            mkdir(dict_FVF_folder)
            
            computeCompartmentSignalFromModelsWithLorentzianCorrection(model_folder, signal_component_FVF_path, model_suffix, dict_params);
            
            % To be used instead of the previous line if you want to
            % parallized the computation
            % jobId1 = qsubfeval(@computeCompartmentSignalFromModelsWithLorentzianCorrection, model_folder, signal_component_FVF_path, model_suffix, dict_params, 'memreq',  req_mem1,  'timreq',  req_etime1);
...
concatenateDictionaryParts(dict_folder, FVFRange, prefix_name, nb_replica, nb_rotations, nb_TE, experience_name, dict_suffix);
