...
for k = 1:size(sub_myelin_x,1)
    susceptibility_Z(sub_myelin_x(k), sub_myelin_y(k)) = model_parameters.current_dir * squeeze(total_X(sub_myelin_x(k), sub_myelin_y(k),:,:)) * model_parameters.current_dir'; 
end
end