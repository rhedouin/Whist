function [signal, field] = simulateSignalFromModel_v2(axon_collection, model_parameters)
...
for l = 1:N    
    C.intra_axonal = exp(1i*model_parameters.TE(l)*omega(axon_index));
    C.myelin = exp(1i*model_parameters.TE(l)*omega(myelin_index));
    C.extra_axonal = exp(1i*model_parameters.TE(l)*omega(extra_index));
