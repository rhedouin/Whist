...
for l = 1:length(parameter_list)
    ...
    if (parameter == 'T2Myelin')
        
    end
    for k = 1:length(orientation_list)
        current_orientation = orientation_list{k};
        
        parameter_map(:,:,:,k) = load_nii_img_only([flip_angle_folder 'BrainSample-2_ses-03_' parameter '_' fa '_polyfit_cartesian_with_theta_noise4_register.nii.gz']);
    end
end
