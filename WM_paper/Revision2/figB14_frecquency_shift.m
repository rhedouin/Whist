...
proportion_list = [0.5 0.7];
% proportion_list = [0.5];

ny_layer_list = [5 10 15 20];
...
for kPorportion = 1:length(proportion_list)
    ...
    for kNbLayer = 1:length(ny_layer_list)
        nb_layer = ny_layer_list(kNbLayer);
        ...
        for kTheta = 1:length(theta_list)
            ...
            options.LineWidth = 2;
            options.plot= 0;
           [~, myelin_layer_mean_shift(kPorportion, kNbLayer, kTheta), myelin_layer_pos_shift(kPorportion, kNbLayer, kTheta), myelin_layer_neg_shift(kPorportion, kNbLayer, kTheta)] = createHistogramFieldPerturbationWithMyelinWater(new_model, myelin_layer_field, options);
...
%     plot(x, original_mean_shift(kPorportion, :),'LineWidth',2);
    ...
    for kNbLayer = 1:length(ny_layer_list)
        plot(x, squeeze(myelin_layer_mean_shift(kPorportion, kNbLayer, :)),'LineWidth',2);
    end
