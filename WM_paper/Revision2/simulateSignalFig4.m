...
model_path = '/project/3015069.04/WM_Models/N400/FVF60_N400_train1/FVF60_gRatio80_N400_train1.mat';
...
xi_list = [-0.2, -0.1, 0, 0.1];

for k = 1:4
    model_parameters.myelin.xi = xi_list(k);  % myelin anisotropic susceptibility (ppm)
...
%     figure(1)
%     hold on
%     plot(model_parameters.TE, abs(signal_original.total_normalized),  'LineWidth', line_width);
%     xlabel('time (ms)')
%     ylabel('|S(t)|')
...
%     figure(2)
%     hold on
%     plot(model_parameters.TE, angle(signal_original.total_normalized),  'LineWidth', line_width);
%     xlabel('time (ms)')
%     ylabel('phase(S(t))')
%     ylim([-3 0.5])
%     set(gca, 'FontSize', 20, 'FontWeight','bold' )
    ...
    figure(3)
    hold on
    plot(model_parameters.TE, abs(signal_corrected.total_normalized),  'LineWidth', line_width);
    xlabel('time (ms)')
    ylabel('|S(t)|')
    set(gca, 'FontSize', 22, 'FontWeight','bold' )
    ...
    figure(4)
    hold on
    plot(model_parameters.TE, angle(signal_corrected.total_normalized),  'LineWidth', line_width);
    xlabel('time (ms)')
    ylabel('phase(S(t))')
    ylim([-3 0.5])
    set(gca, 'FontSize', 22, 'FontWeight','bold' )
