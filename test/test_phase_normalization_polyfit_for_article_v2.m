...
for l = 1:lTime
    ...
    for k = 0
        ...
        plot(1:nbTE, real_original_signal, 'LineWidth', 0.5);
        plot(nbTE_max+3 : nbTE_max+nbTE+2, imag_original_signal, 'LineWidth', 0.5);
        xline(nbTE_max + 1.5);
        xticks([1, nbTE-1, nbTE_max+4, nbTE_max+nbTE+1])        
        ...
        subplot(1,2,2)
        hold on
        plot(1:nbTE, real_norm_poly, 'LineWidth', 0.5);
        plot(nbTE_max+3 : nbTE_max+nbTE+2, imag_norm_poly, 'LineWidth', 0.5);
        xline(nbTE_max + 1.5);
        xticks([1, nbTE-1, nbTE_max+4, nbTE_max+nbTE+1])       
        xticklabels({time_ms(1),time_ms(end),time_ms(1),time_ms(end)})
        xlabel('Echo time (ms)')
