...
list_fa = ['fa-5', 'fa-10', 'fa-20', 'fa-30', 'fa-40', 'fa-50', 'fa-70']
#list_fa = ['fa-60']
...
for kFa in range(0, nbFa):
	...
	base_signal_name = 'BS-3_ses-mri03_acq-lowres'
	base_signal_folder = os.path.join(data_project_folder, 'derived/BS-3/ses-mri03/gre/lowres/ref_space/concatenate_signals/')
	print(base_signal_folder)

	dp_project_folder = '/project/3015069.04/'
	base_dp_folder = os.path.join(dp_project_folder, 'deep_learning/multi_orientations/BS-3/')
	print(base_dp_folder)

	dict_base_name = 'SignalWithNoise' + noise + '_8rep_10rotations_18TE_BS-3_polyfit_cartesian_with_theta'
	...
	input_mask_nii_path = os.path.join(base_signal_folder, 'BS-3_ses-mri03_acq-lowres_gre_gradunwarp_mask_sphere_ref.nii.gz')
	...
	output_parameter_folder = os.path.join(base_signal_folder, 'parameter_maps/noise' + noise + '/' + fa)
